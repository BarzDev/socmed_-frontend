  ...
  return (
    <div className="min-h-screen font-bold">
      <div className="py-16 flex items-center justify-center gap-2 bg-sky-500 mb-3">
        <Avatar name="Kent Dodds" src={session?.user?.photo ?? undefined} />

        <p className="text-white">{session?.user?.username ?? "Guest"}</p>
      </div>

      <div className="flex flex-col gap-3 ps-5 mb-20">
        {links.map((link) => {
          return (
            <Link
              href={link.href}
              key={link.name}
              className={clsx(
                " flex gap-2 p-2 rounded-l-lg hover:text-sky-500",
                {
      ...
      <div className="ps-5">
        <button
          onClick={() => signOut()}
          className="flex  gap-2 text-white p-2 rounded-lg bg-red-500 hover:bg-red-600"
        >
          {logoutIcon} Logout
        </button>
      </div>
